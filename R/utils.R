...
is_leap <- function(year){
  leap <- case_when(year %% 400 == 0 ~ TRUE,
                    year %% 100 == 0 ~ FALSE,
                    year %% 4   == 0 ~ TRUE,
                    TRUE ~ FALSE)
  leap
...
month_length <- function(month, leap = FALSE){
  num_days <- case_when(month == 1 & leap         ~ 29,
                        month == 1                ~ 28,
                        month %in% c(3, 5, 8, 10) ~ 30,
                        TRUE                      ~ 31)
  num_days
}

calendar_fractions <- function(start_day,
                               end_day,
                               start_length,
                               end_length){
  case_when(end_day > start_day ~ (end_day - start_day) / end_length,
            end_day < start_day ~ (start_length - start_day) / start_length +
                                  end_day / end_length,
            TRUE ~ 0)
}
...
age_calc <- function (dob, enddate = Sys.Date(),
  ...
  if (precise) {
    start_is_leap <- is_leap(start$year)
    end_is_leap <- is_leap(end$year)
  }
  ...
  if (units == "months") {
    months <- sapply(mapply(seq,
                     ...
                     length) - 1
    if (precise) {
      month_frac <- calendar_fractions(start$mday,
                                       end$mday,
                                       month_length(end$mon - 1, start_is_leap),
                                       month_length(end$mon, end_is_leap))
  ...
  if (units == "years") {
    ...
    if (precise) {
      ...
      year_frac <- calendar_fractions(start$yday,
                                      end$yday,
                                      start_length,
                                      end_length)
...
#' @param n number of elements to generate, usually defined by \link[dplyr]{n} in dplyr
#' @param lst probability list that contains function and function parameters
#' @param ... additional arguments passed into the function
